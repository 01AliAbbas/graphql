        ...
        <Grid container spacing={2} sx={{ minHeight: "100%" }}>
            ...
            <Grid item xs={12} md={6}>
                <Card sx={{ backgroundColor: "rgba(0, 255, 255, 0.1)", color: "#fff", p: 3, borderRadius: 2, minHeight: "40vh" }}>
                    <CardContent>
                        <Typography variant="h6" sx={{ color: "white", mb: 1 }}>User Details</Typography>
                        {
                            (
                                <>
                                    <Typography sx={{ color: "white" }}>
                                        Name: <span style={{ color: "#0ff" }}>{user?.firstName} {user?.lastName}</span>
                                    </Typography>
                                    <Typography sx={{ color: "white" }}>
                                        Email: <span style={{ color: "#0ff" }}>{user?.email}</span>
                                    </Typography>
                                    <Typography sx={{ color: "white" }}>
                                        Joined At: <span style={{ color: "#0ff" }}>{new Date(user?.createdAt).toLocaleDateString()}</span>
                                    </Typography>
                                    <Typography sx={{ color: "white" }}>
                                        Level: <span style={{ color: "#0ff" }}>{user?.events[0]?.level}</span>
                                    </Typography>
